...
$developers_posts = get_posts(array('post_type' => 'houzez_developer', 'posts_per_page' => -1, 'suppress_filters' => 0));
...
$developers_array = isset($prop_meta_data['fave_developers']) ? $prop_meta_data['fave_developers'] : array();
...
if($agent_display_option == 'agent_info') {
	...
	$agency_hidden_class = '';
} else if($agent_display_option == 'developer_info') {
	$agent_hidden_class = '';
} else if($agent_display_option == 'developer_info') {
	$agency_hidden_class = '';
}
...
<div class="form-group">
	...
	<label class="control control--checkbox">
		<input <?php checked($agent_display_option, 'developer_info', true); ?> type="radio" value="developer_info" name="fave_agent_display_option"> <?php echo houzez_option('cl_developer_info', 'Developer Info (Choose developer from the list below)'); ?>
		<span class="control__indicator"></span>
	</label>

	<div class="developers-dropdown form-group ml-2 form-group col-sm-12 col-md-5">
		<select name="fave_developers[]" class="selectpicker form-control bs-select-hidden" title="<?php echo houzez_option('cl_developer_info_plac', 'Select an Developer'); ?>" data-live-search="true" data-selected-text-format="count > 2"  data-actions-box="false" multiple>
			<?php

			if ( houzez_is_agency() ) {
	            if (!empty($developers_posts)) {
	            	$i = 0;
	            	$output = '';
	            	$agency_id = get_user_meta(get_current_user_id(), 'fave_author_agency_id', true);
	                foreach ($developers_posts as $developer_post) {
	          	
	          			if( $agency_id == get_post_meta($developer_post->ID, 'fave_agent_agencies', true) ) {
		                    $output .= '<option ';

		                    if(houzez_edit_property()) {
		                    	if( in_array($developer_post->ID, $developers_array) ) {
		                    		$output .= ' selected';
		                    	}
		                    }

		                    $output .= ' value="'.$developer_post->ID.'">'.$developer_post->post_title.'</option>';
		                }
